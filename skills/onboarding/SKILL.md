...
1. From the project root, run the bundled script:
   ...
   ```bash
   node <this-skill-dir>/onboarding.mjs
   ```
...
2. If you can't run Node interactively, do **guided onboarding** instead: run `node onboarding.mjs --detect-only`, then walk the user through each field (see the sections in the generated `stack.md`) and write `.claude/stack.md` + `.claude/stack.json` yourself from their answers.
