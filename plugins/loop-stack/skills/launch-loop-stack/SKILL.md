...
5. **Create each missing, applicable loop** with `CronCreate` (`recurring: true`) using the cron expression + the resolved prompt. The prompt IS the per-tick instruction fired into a fresh session.
   - **A denied `CronCreate` is not a verdict — retry it once.** Permission classifiers score the prompt text, and these prompts are long and full of `git`/`gh` verbs, so denials are non-deterministic: the *same* prompt that was blocked frequently succeeds on a second call. Retry each denial once before treating the loop as unlaunched. Do not reword the prompt to get past a denial — a reworded loop is a different loop, and the words it loses (`never override branch protection`, the overlap guard) are the safety ones.
   - Still denied after the retry → that loop is **unlaunched**. Carry it into step 6; never silently continue.
6. **Verify what actually exists — `CronList` before reporting.** Never report from the return values of your `CronCreate` calls; list the jobs and diff against the set of loops you intended to launch. Any intended loop with no matching job is **MISSING**.
7. **Report** every job ID created (and any skipped — as duplicate, or as N/A for this project's config), the cadence, that they're session-only + auto-expire in 7 days, and how to stop (`stop-loop-stack` / `CronDelete <id>` / `CronList`).
   - **A partial stack is the dangerous outcome, so say so in those words.** The surviving loops keep ticking and keep reporting "STOP — no action", which reads exactly like a healthy idle stack; nothing about the steady-state output reveals that a queue is unwatched. For each MISSING loop, name **the work it owns and that nothing is now watching** — e.g. "FIX is missing: new bugs in `${states.todo}` will not be picked up by anything"; "PR-SHEPHERD is missing: failing checks and review comments on my PRs will sit untouched". A bare "3 loops failed to start" is not enough.
   - Then stop and let the user choose: re-run this skill, change permission mode, add a settings rule, or run the missing loop's work on demand.
8. Don't run a tick inline now unless asked — the crons fire on their own schedule.
